...
if (!window || !window.console) {
    console = {tiddlywiki: true, log: function (message) {
...
TiddlyWiki.prototype.sortTiddlers = function (tiddlers, field) {
    ...
    if (TiddlyWiki.standardFieldAccess[field]) {
        if (field == "title") {
            tiddlers.sort(function (a, b) {
                return a[field].toLowerCase() < b[field].toLowerCase() ? -asc : (a[field].toLowerCase() == b[field].toLowerCase() ? 0 : asc);
            });
        } else {
            tiddlers.sort(function (a, b) {
...
config.macros.importTiddlers.restart = function (wizard) {
    ...
    wizard.setButtons([
        {caption: this.openLabel, tooltip: this.openPrompt, onClick: me.onOpen}
    ]);
...
config.macros.importTiddlers.onGetWorkspaceList = function (context, wizard) {
    ...
    wizard.setButtons([
        {caption: me.openLabel, tooltip: me.openPrompt, onClick: me.onChooseWorkspace}
    ]);
};
...
config.macros.upgrade.handler = function (place) {
    ...
    w.setButtons([
        {caption: this.upgradeLabel, tooltip: this.upgradePrompt, onClick: this.onClickUpgrade}
    ]);
};
...
function javaDebugInformation() {
    ...
    return jQuery.map(what, function (item) {
        return formatItem.apply(this, item)
    })
        .join('\n\n')
}
...
FileAdaptor.getTiddlerListComplete = function (context, userParams) {
    if (context.status) {
        if (context.filter) {
            ...
            context.tiddlers = [];
            context.adaptor.store.forEachTiddler(function (title, tiddler) {
